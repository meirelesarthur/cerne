    ...
    },
    {
      ...
      render: (e) => (
        <div style={{ display: 'flex', justifyContent: 'center' }} title={e.emiteNfe === 'sim' ? 'Emite NFe' : 'Não emite NFe'}>
          {e.emiteNfe === 'sim'
            ? <Icon name="check" size={16} color={t.color.feedback.success.text} aria-label="Emite NFe" />
            : <Icon name="close" size={16} color={colors.fg.subtle} aria-label="Não emite NFe" />}
        </div>
      ),
    },
    {
      ...
      render: (e) => (
        <div onClick={ev => ev.stopPropagation()} style={{ display: 'flex', justifyContent: 'center' }}>
          <DropdownMenu
            ...
            items={[
              { id: 'view', label: 'Visualizar', icon: <Icon name="view" size={13} />, onClick: () => onView(e.id) },
              { id: 'edit', label: 'Editar', icon: <Icon name="edit" size={13} />, onClick: () => onEdit(e.id) },
              { id: 'cert', label: 'Certificado', icon: <Icon name="document-key" size={13} />, onClick: () => onCertificado(e.id) },
              { id: 'delete', label: 'Excluir', icon: <Icon name="delete" size={13} />, onClick: () => setDeleteId(e.id), danger: true, divider: true },
            ]}
          />
        </div>
      ),
