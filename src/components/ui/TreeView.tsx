        ...
        return (
          <li key={node.id} role="treeitem" aria-expanded={hasChildren ? isExpanded : undefined} aria-selected={selected}>
            <div style={{ display: 'flex', alignItems: 'center', gap: t.space[1], minHeight: t.size.control, padding: `${t.space[1]}px ${t.space[2]}px ${t.space[1]}px ${depth * t.space[5] + t.space[2]}px`, borderRadius: t.radius.base, background: selected ? colors.accent.subtle : 'transparent' }}>
              {hasChildren ? (
                <IconButton icon={isExpanded ? <Icon name="chevron-down" size={t.icon.xs} /> : <Icon name="chevron-right" size={t.icon.xs} />} aria-label={isExpanded ? `Recolher ${node.label}` : `Expandir ${node.label}`} size="xs" variant="ghost" onClick={() => toggle(node.id)} />
              ...
              {onEdit && <IconButton icon={<Icon name="edit" size={t.icon.xs} />} aria-label={`Editar ${node.label}`} size="sm" variant="ghost" onClick={() => onEdit(node)} />}
              {canAdd && <IconButton icon={<Icon name="add" size={t.icon.xs} />} aria-label={`Criar descendente em ${node.label}`} size="sm" variant="ghost" onClick={() => onAddChild(node, depth)} />}
              {onDelete && <IconButton icon={<Icon name="delete" size={t.icon.xs} />} aria-label={hasChildren ? `${node.label} possui descendentes e não pode ser excluído` : `Excluir ${node.label}`} size="sm" variant="ghost" danger disabled={hasChildren} onClick={() => onDelete(node)} />}
            </div>
